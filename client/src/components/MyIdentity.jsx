...
export default function MyIdentity({ identity, onRegenerate, onShowToast }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Top Header Card */}
      <div className="p-6 sm:p-8 bg-[#111c14] border border-[#24392b] rounded-2xl shadow-xl">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8 justify-between">
          {/* Left: QR Display */}
          <div className="flex flex-col items-center gap-3">
            <div className="p-3 sm:p-4 bg-white rounded-2xl border-2 border-[#8fe3a0] shadow-[0_0_20px_rgba(143,227,160,0.2)] relative">
              <QRCodeSVG
                id="identity-qr-code"
                value={qrPayload}
                size={220}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <Download className="w-3.5 h-3.5 text-[#8fe3a0]" />
                <span>Save QR</span>
              </button>
              <button
              ...
              >
                {copiedField === 'payload' ? (
                  <Check className="w-3.5 h-3.5 text-[#8fe3a0]" />
                ) : (
                  <Share2 className="w-3.5 h-3.5 text-[#8fe3a0]" />
                )}
          ...
          <div className="flex-1 w-full space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-[#5c9a6b] mt-0.5 font-mono">
                SHA-256 digest of your combined ECDSA &amp; ECDH public keys.
              </p>
            </div>

            {/* Fingerprint Box */}
            <div className="bg-[#0b120e] border border-[#24392b] rounded-xl p-3 relative group">
              <div className="font-mono text-[#8fe3a0] font-semibold text-xs sm:text-sm tracking-wider break-all select-all pr-12">
                {identity.fingerprint}
              </div>
              <button
                onClick={() => handleCopy(identity.fingerprint, 'fingerprint', 'Fingerprint copied')}
                className="absolute right-2.5 top-2.5 p-1.5 rounded-lg bg-[#111c14] border border-[#24392b] text-[#5c9a6b] hover:text-[#8fe3a0] shadow-xs cursor-pointer transition-colors"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              <div className="p-3 bg-[#0b120e] border border-[#24392b] rounded-xl">
                <div className="text-xs text-[#5c9a6b] font-medium font-mono">Signing Algorithm</div>
                <div className="text-sm font-semibold text-white mt-0.5 font-mono">ECDSA NIST P-256</div>
                <div className="text-[11px] text-[#8fe3a0] mt-0.5 font-mono">Non-extractable Private Key</div>
              </div>
              <div className="p-3 bg-[#0b120e] border border-[#24392b] rounded-xl">
                <div className="text-xs text-[#5c9a6b] font-medium font-mono">Key Agreement</div>
                <div className="text-sm font-semibold text-white mt-0.5 font-mono">ECDH NIST P-256</div>
                <div className="text-[11px] text-[#8fe3a0] mt-0.5 font-mono">Non-extractable Private Key</div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-1">
              <button
                ...
                className="btn btn-primary text-xs py-2.5 px-4 flex items-center gap-1.5 flex-1 justify-center"
              >
                {copiedField === 'raw_json' ? (
                  <>
                    <Check className="w-3.5 h-3.5 text-[#0b120e]" />
              ...
              <button
                ...
                className="btn btn-danger text-xs py-2.5 px-3.5 flex items-center gap-1.5 justify-center"
      ...
      {showConfirmModal && (
        <div 
          ...
          aria-modal="true"
        >
          <div className="bg-[#131f17] border border-[#24392b] max-w-sm w-full p-5 rounded-2xl space-y-4 shadow-2xl animate-scale-up">
            <div className="flex items-center gap-2.5 text-rose-400">
              <AlertTriangle className="w-5 h-5" />
              <h3 className="text-base font-bold text-white font-mono">Regenerate Identity?</h3>
            </div>

            <p className="text-xs text-[#7fa889] leading-relaxed">
